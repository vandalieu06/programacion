...
SELECT first_name, last_name, email FROM employees WHERE salary >= 12000;

-- 2. Nom i Cognom dels empleats contractats entre l’any 2007 i 2009.
--SELECT first_name, last_name FROM employees WHERE hire_date BETWEEN '01-01-2007' AND '31-12-2009';
...
SELECT job_title FROM jobs WHERE min_salary BETWEEN 5000 AND 10000; 

-- 4. Nom, Cognom, sou, sou anual (12 mesos). Reanomena l’última columna com a ‘salari12’.
SELECT first_name, last_name, salary, salary * 12 AS "salari12" FROM employees;

--5. En una sola columna mostra la frase: <nom> <cognom> cobra <salari> dòlars mensuals
--   i va ser contractat al mes de <mes on va ser contractat> . Reanomena la columna com a ‘La Frase’ .
...
SELECT job_id, job_title FROM jobs WHERE LOWER(job_title) LIKE '%manager%';
...
SELECT department_name FROM departments WHERE UPPER(department_name) LIKE 'C%N'; 

-- 8. Mostra els noms de departament que tenen un nom compost (dues paraules o més) i que tenen MANAGER assignat.
SELECT department_name FROM departments WHERE department_name LIKE '% %' AND manager_id IS NOT NULL;

-- 9. Mostra a quins països (Countries) corresponen els ids (CA, KW i MX). Mostra el nom enmajúscules.
SELECT country_name FROM countries WHERE country_id IN ('CA', 'KW', 'MX');
...
SELECT first_name, ROUND((salary / 30 * 0.94), 2) || '€' AS "Salari arrodonit" FROM employees; 
...
SELECT department_name FROM departments WHERE (location_id = 1700 OR department_id = 20) AND manager_id IS NOT NULL;
...
SELECT  first_name, LPAD(salary, 6, '0') AS "SALARI6" FROM employees;

-- 15. Mostra el salari màxim, mínim i mitjà, dels empleats que treballen al departament 100.
SELECT MAX(salary) AS "MAX", MIN(salary) AS "MIN", ROUND(AVG(salary), 2) AS "MIDDLE" FROM employees WHERE DEPARTMENT_ID = 100;

-- 16. Mostra els noms de tots els empleats i quantes vegades està repetit cada nom.
SELECT first_name, COUNT(first_name) as "REPEAT_NAME" FROM employees GROUP BY first_name; 
